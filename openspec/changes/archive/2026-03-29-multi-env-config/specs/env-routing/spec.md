...
- **THEN** 每份文件 SHALL 各自包含完整的 build host（REMOTE_HOST、REMOTE_PORT、REMOTE_USER、SSH_KEY_PATH、REMOTE_BASE_DIR）、Harbor（HARBOR_HOST、HARBOR_PROJECT）和 deploy host（DEPLOY_HOST、DEPLOY_PORT、DEPLOY_USER、DEPLOY_SSH_KEY_PATH、DEPLOY_BASE_DIR）配置

#### Scenario: env 文件格式与 remote.env 一致
- **WHEN** AI 层将 env 文件路径传给 `build.sh --config`
- **THEN** 脚本层 SHALL 能正常 `source` 该文件，无需格式适配

### Requirement: projects.yaml env 字段
`projects.yaml` 中每个项目条目 SHALL 支持顶层 `env` 字段，值为环境名字符串（对应 `remote-envs/{env}.env`）。字段可选，缺省时表示使用默认 `remote.env`。
...
#### Scenario: 项目未声明 env
- **WHEN** 项目条目不含 `env` 字段或值为空
- **THEN** AI 层 SHALL 回退使用 `image-builder/remote.env` 作为配置来源
...
#### Scenario: 无 env 字段时使用默认配置
- **WHEN** AI 层构建项目，该项目无 `env` 字段
- **THEN** AI 层 SHALL 不传 `--config` 参数（`build.sh` 使用默认 `remote.env`）
...
`CLAUDE.md` SHALL 在"职责分层"或相关章节中补充多环境路由的 AI 层行为说明，确保后续 AI 会话能正确执行环境路由。

#### Scenario: AI 层行为文档化
- **WHEN** 新的 AI 会话读取 CLAUDE.md
- **THEN** SHALL 能理解：项目有 `env` 字段时需加载对应 env 文件并传 `--config`，无该字段时使用默认配置
