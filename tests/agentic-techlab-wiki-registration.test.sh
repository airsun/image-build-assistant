...
OFFICE32_ENV="${ASSISTANT_ROOT}/image-builder/remote-envs/office-32.env"
...
resolve_project_by_name "${REGISTRY}" agentic-techlab-wiki-infra office-32
...
assert_eq "${ENV_NAME}" office-32 "environment"
...
artifact_evidence="${SOURCE_DIR}/docs/verification/2026-08-21-office32-artifact.json"
artifact_commit="$(ruby -rjson -e 'puts JSON.parse(File.read(ARGV.fetch(0))).fetch("image").fetch("sourceCommit")' "${artifact_evidence}")"
assert_eq "${BUILT_COMMIT}" "${artifact_commit}" "artifact source commit"
[[ -z "$(git -C "${SOURCE_DIR}" status --short)" ]] || fail "B source worktree must be clean"

# shellcheck disable=SC1090
source "${OFFICE32_ENV}"
assert_eq "${REMOTE_HOST}" 192.168.14.32 "remote host"
...
printf 'PASS: agentic-techlab-wiki-infra Office-32 registration\n'
